...
public class day7year2025 {
    public static void main(String[] args) {
        ...
        System.out.println((long) part2(grid));
    }
    ...
    public static double part2(String[][] grid) {
        // change data file first line from S to |
        // part 2 easier than part 1 imo since coding part 1 was harder
        ArrayList<Double> position = new ArrayList<>();
        for (int i = 0; i < grid.length; i++) {
            position.add(0.0);
        }
        position.set(position.size() / 2 - 1, 1.0);
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                System.out.print(grid[i][j]);
                if (grid[i][j].equals("^")) {
                    position.set(j - 1, position.get(j - 1) + position.get(j));
                    position.set(j + 1, position.get(j + 1) + position.get(j));
                    position.set(j, 0.0);
                }
            }
